...
export default function TemplatesPage() {
    ...
    const [masterDiagnoses, setMasterDiagnoses] = useState<MasterDiagnosis[]>(MOCK_DIAGNOSES);
    ...
    const handleSaveMasterDiagnosis = (updatedDiagnosis: MasterDiagnosis) => {
        setMasterDiagnoses(prev =>
        prev.map(d => (d.id === updatedDiagnosis.id ? updatedDiagnosis : d))
        );
        toast({
        title: 'Master Diagnosis Updated',
        description: `"${updatedDiagnosis.clinicalDiagnosis}" has been updated. (This is a mock save)`,
        });
    };

    const handleAddNewMasterDiagnosis = (data: Omit<MasterDiagnosis, 'id'>) => {
