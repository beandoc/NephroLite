  ...
  const chartData = useMemo(() => {
    ...
    if (medicationHistory) {
      medicationHistory.forEach(visit => {
        ...
        if (visit.clinicalData?.uacr) {
          // Prefer investigation record if present
          if (!groupedByDate[dateKey].uacr) {
            groupedByDate[dateKey].uacr = parseFloat(visit.clinicalData.uacr);
          }
        }
        // Check if "proteinuria" or "24h protein" is in clinicalData if we added it?
        // Schema only has 'uacr', 'serumCreatinine', 'totalCholesterol', 'hdlCholesterol' added.
        // We didn't add 24h protein field to the form yet.
      });
    }
