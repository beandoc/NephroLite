  ...
  const chartData = useMemo(() => {
    ...
    if (patient.visits) {
      patient.visits.forEach(visit => {
        if (visit.clinicalData?.serumCreatinine) {
          const dateKey = format(parseISO(visit.date), 'yyyy-MM-dd');
          if (!groupedByDate[dateKey]) {
            groupedByDate[dateKey] = {};
          }
          // Prefer investigation record if both exist for same date (unlikely but possible)
          if (!groupedByDate[dateKey].creatinine) {
            groupedByDate[dateKey].creatinine = parseFloat(visit.clinicalData.serumCreatinine);
          }
        }
      });
    }
